...
# --- THE FINAL FIXED PDF LOGIC ---
def create_pdf(proj_name, total_irecs, revenue, expenses, net_profit, years):
    pdf = FPDF()
    pdf.add_page()
    
    # Header
    pdf.set_font("Helvetica", "B", 16)
    pdf.cell(0, 10, f"I-REC Strategic Valuation: {proj_name}", ln=True, align='C')
    pdf.set_font("Helvetica", "I", 10)
    pdf.cell(0, 10, "Generated by Triara CAP", ln=True, align='C')
    pdf.ln(10)
    ...
    pdf.cell(0, 8, f"Project Period: {years} Year(s)", ln=True)
    pdf.cell(0, 8, f"Total Estimated I-RECs: {int(total_irecs):,}", ln=True)
    pdf.ln(5)

    # Financials
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(100, 10, "Component", border=1)
    pdf.cell(80, 10, "Value (INR)", border=1, ln=True)
    
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(100, 10, "Gross Revenue", border=1)
    pdf.cell(80, 10, f"{int(revenue):,}", border=1, ln=True)
    pdf.cell(100, 10, "Total Expenses & Fees", border=1)
    pdf.cell(80, 10, f"{int(expenses):,}", border=1, ln=True)
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(100, 10, "Net Client Profit", border=1)
    pdf.cell(80, 10, f"{int(net_profit):,}", border=1, ln=True)

    # Return as bytes
    return bytes(pdf.output())

# --- BUTTON LOGIC ---
# Place this exactly where you want the button to appear
try:
    pdf_bytes = create_pdf(proj_name, total_irecs_period, gross_revenue, total_period_expenses, client_net_profit, projection_years)
    
    st.sidebar.download_button(
        label="📩 Download PDF Report",
        data=pdf_bytes,
        file_name=f"ABR_IREC_Report.pdf",
        mime="application/pdf"
    )
except Exception as e:
    st.sidebar.error("PDF Generation Error. Please check library versions.")
