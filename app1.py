...
def create_pdf(proj_name, total_irecs, revenue, expenses, net_profit, years):
    ...
    pdf.cell(0, 15, f"I-REC STRATEGIC VALUATION: {proj_name}", ln=True, align='C')
    pdf.set_font("Arial", "I", 10)
    pdf.cell(0, 10, "CONFIDENTIAL REPORT - GENERATED BY TRIARA CAP", ln=True, align='C')
    pdf.ln(10)
    ...
    pdf.cell(0, 10, f"Projection Horizon: {years} Year(s)", ln=True)
    pdf.cell(0, 10, f"Total I-RECs Generated: {int(total_irecs):,}", ln=True)
    pdf.ln(5)

    # Financial Data
    pdf.set_font("Arial", "B", 12)
    pdf.cell(90, 10, "Metric", border=1)
    pdf.cell(90, 10, "Amount (INR)", border=1, ln=True)
    
    pdf.set_font("Arial", "", 12)
    pdf.cell(90, 10, "Total Gross Revenue", border=1)
    pdf.cell(90, 10, f"INR {int(revenue):,}", border=1, ln=True)
    pdf.cell(90, 10, "Total Expenses & Fees", border=1)
    pdf.cell(90, 10, f"INR {int(expenses):,}", border=1, ln=True)
    
    pdf.set_font("Arial", "B", 12)
    pdf.cell(90, 10, "Net Client Profit", border=1)
    pdf.cell(90, 10, f"INR {int(net_profit):,}", border=1, ln=True)
    
    return pdf.output(dest='S').encode('latin-1')

# --- DOWNLOAD BUTTON IN SIDEBAR ---
pdf_data = create_pdf(proj_name, total_irecs_period, gross_revenue, total_period_expenses, client_net_profit, projection_years)

st.sidebar.download_button(
    label="📩 Download Full Proposal (PDF)",
    data=pdf_data,
    file_name=f"ABR_IREC_Valuation_{proj_name}.pdf",
